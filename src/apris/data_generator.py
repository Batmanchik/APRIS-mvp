...
def build_dataset(
    ...
    if enforce_training_size:
        if not (3000 <= total_n <= 5000):
            raise ValueError("Dataset size must be in range [3000, 5000].")
    else:
        if not (200 <= total_n <= 5000):
            raise ValueError("Dataset size must be in range [200, 5000].")
    half = total_n // 2
    legit_n = half
...
def validate_dataset(
    ...
    for feature, (low, high) in FEATURE_BOUNDS.items():
        ...
        if not (values >= low).all():
            raise ValueError(f"{feature} has values below {low}")
        if not (values <= high).all():
            raise ValueError(f"{feature} has values above {high}")
    ...
    if not (0.48 <= pyramid_share <= 0.52):
        raise ValueError("Label distribution is not close to 50/50.")
    ...
    for feature in FEATURE_COLUMNS:
        ...
        if not (overlap_ratio > min_overlap_ratio):
            raise ValueError(
