"""
Дашборд аномалий (очередь алертов).
...
with c2:
    st.markdown(f"<div class='metric-card'><div class='metric-val red-val'>{critical}</div><div class='metric-label'>Критических (срочная проверка)</div></div>", unsafe_allow_html=True)
with c3:
...
with c4:
    st.markdown(f"<div class='metric-card'><div class='metric-val' style='color:#10a37f;'>{total_scanned - critical - suspicious}</div><div class='metric-label'>Низкий риск (базовая зона)</div></div>", unsafe_allow_html=True)

st.markdown("---")
st.subheader("📋 Лидерборд рисков (очередь алертов)")
    ...
    with st.container():
        ...
        with col_summary:
            ...
            try:
                ...
                st.markdown("#### 🚩 Топ-3 индикатора (почему поднят сигнал):")
                ...
                st.warning("Не удалось получить объяснение с сервера API.")

        with col_ml:
            st.markdown("#### 🕸️ Крипто-сеть транзакций (подозрительный шаблон)")

            preset = "PYRAMID" if prob >= 0.7 else ("DANGEROUS" if prob >= 0.4 else "SAFE")

            with st.spinner("Построение транзакционного графа смарт-контракта..."):
                fake_company_name = f"Смарт-контракт {selected_id[:6]}"
